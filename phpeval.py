...
def trim(str_trim):
    str_trim = re.sub(r'^(\s)*', '', str_trim)
    str_trim = re.sub(r'(\s)*$', '', str_trim)
    return str_trim

# remove quotes around a string ' and "
def decoted(str_decote):
    str_decote = re.sub(r"(\"|\')(\s)*$", '', str_decote)
    str_decote = re.sub(r"^(\s)*(\"|\')", '', str_decote)
    return str_decote
...
def evaluate(strline):
    ...
    # Eval()    
    elif re.match(r'^eval(\s)*\(.*\)' ,strline):
    ...
    # gzinflate()
    elif re.match(r"^gzinflate(\s)*\((?P<CODE>.*)\)", strline):
        ...
        if re.match(r'^\$', code):  # gzinflate variable
            func_payload = get_phpvar(code) # from previous set variable
        elif re.match(r"^['\"]",code ): # else load from direct code
            func_payload = code
        func_payload = zlib.decompressobj().decompress('x\x9c' + func_payload)

    # base64_decode
    elif re.match(r"^base64_decode(\s)*\((?P<CODE>.*)\)", strline):
    ...
    # str_rot13
    elif re.match(r"^str_rot13(\s)*\((?P<CODE>.*)\)", strline):
        ...
        if re.match(r'^\$', code):  # if value is a variable
            func_payload = get_phpvar(code) # from previous set variable
        elif re.match(r"^['\"]",code ): # else load from direct code
            func_payload = code
    ...
    # strrev    
    elif re.match(r"^strrev(\s)*\((?P<CODE>.*)\)", strline):
        ...
        if not re.match(r"^['\"\$]",code ):
            ...
            evaluate(code)
        # No more nested function
        if re.match(r'^\$', code):  # gzinflate variabl
            func_payload = get_phpvar(code) # from previous set variable
    ...
    for char in Result:
        if char == chr(0x0A) :
          # Chomp line    
            ...
            tmpbuffer = re.sub(r'\s+',     ' ', tmpbuffer)
            tmpbuffer = re.sub(r'^\s',     '', tmpbuffer)
            if tmpbuffer != "":
...
# How many eval found
EVALFOUND = len(re.findall(rb"eval(\s)*\(", byteArr))
if EVALFOUND == 0 :
